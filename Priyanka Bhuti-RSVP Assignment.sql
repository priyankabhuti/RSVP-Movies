...
FROM movie;
-- country, worlwide_gross_income, languages, production_compay columns has null values
...
-- Numbers of movies released each year
SELECT 
      year,
...
SELECT
      COUNT(DISTINCT genre) AS unique_list
FROM genre;

-- There are total 13 genres present in the data set
...
-- Drama had the highest number of movies produced.
...
SELECT 
      g.genre,
      AVG(m.duration) AS avg_genre
FROM movie m
INNER JOIN genre g ON m.id=g.movie_id
GROUP BY genre;

-- drama has the highest avg_duration of 106.7mins.
...
-- Kirket is the top movie based on average rating.
...
-- There are total 8 movies which start with 'The' and 'The Brighton Miracle' has the highest rating
...
-- when we use median_rating inplace of avg_rating we again get total 8 movies that start with 'The' 
-- but this time 'The Blue Elephant 2'has the highest rating
...
-- there are total 361 movies with median rating as 8 and released between 1 April 2018 and 1 April 2019
...
-- German movies get more votes than italian
-- Answer is Yes
...
(SELECT
      ...
      RANK() OVER (ORDER BY COUNT(m.id) DESC) AS ranked
FROM movie m
INNER JOIN genre g   ON m.id=g.movie_id
INNER JOIN ratings r ON r.movie_id=m.id
WHERE avg_rating > 8 
...
FROM  director_mapping d INNER JOIN genre g USING(movie_id)
...
SELECT
      ...
      RANK() OVER (ORDER BY COUNT(r.total_votes) DESC) AS prod_comp_rank
FROM movie m 
INNER JOIN ratings r ON m.id=r.movie_id
GROUP BY production_company
...
SELECT
      ...
      AVG(ROUND(AVG(m.duration),2)) OVER (ORDER BY genre ROWS BETWEEN 3 PRECEDING AND CURRENT ROW) AS moving_avg_duration 
FROM genre g
...
-- the top 3 genre are action, comedy, drama and the top 5 movies amongst them are shown in the result table.
...
-- star cinema and twentieth century fix are the top 2 production houses.
...
-- Parvathy Thiruvothu, Susan Brown and Amanda Lawrence are the top three actresses.
